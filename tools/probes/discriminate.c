 ...
 * tend to diverge. Compiled with each candidate mwccarm version;
 * tools/probe_versions.py flags any function whose bytes differ across
 * versions - narrows down which dsi/ build PictoChat was actually built
 * with (see notes/setup-mwccarm.md; the pin is still just a copyright-date
 * guess until a real construct like one of these splits the field).
 * Adapted from sm64ds-decomp's tools/probes/discriminate.c. */
