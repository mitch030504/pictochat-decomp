// decomp: module=arm7 addr=0x022d6048 name=FUN_022d6048
// GHIDRA DRAFT - a reading aid, NOT a matching candidate.
// size 0x68 (Ghidra's cache says 0x64 - it excludes this function's trailing literal pool), arm
// triage: noise=3 statements=21
// verify:  python tools/match.py --c <file> --func FUN_022d6048 --addr 0x022d6048 --size 0x68 --module arm7 --version 2.0/sp1


void FUN_022d6048(void)

{
  undefined2 *puVar1;
  short *psVar2;
  
  psVar2 = *(short **)(DAT_022d60ac + 0x550);
  if (*psVar2 == 1) {
    thunk_EXT_FUN_03802630(1);
    *psVar2 = 0;
    puVar1 = (undefined2 *)func_0x037d14bc();
    *puVar1 = 4;
    puVar1[1] = 0;
    func_0x037d1464();
  }
  else {
    puVar1 = (undefined2 *)func_0x037d14bc();
    *puVar1 = 4;
    puVar1[1] = 3;
    func_0x037d1464();
  }
  return;
}

